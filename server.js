...
const { saveToSupabase, isSupabaseEnabled } = require('./supabaseStorage');
...
// ── Health ────────────────────────────────────────────────────────
app.get('/health', (_, res) => res.json({ ok: true, version: '2.2.0' }));
...
app.get('/api/config', (_, res) => {
  ...
  res.json({
    ...
    audioMaxMin:  cfg.audioMaxMin  || 10,
    videoPrefix:  cfg.videoPrefix  || 'video',
    videoSizeMb:  cfg.videoSizeMb,
    videoMaxMin:  cfg.videoMaxMin  || 5,
...
app.post('/api/admin/config', adminLimiter, requireAdmin, (req, res) => {
  const allowed = [
    ...
    'tinymceKey','audioPrefix','videoPrefix','adminPassword',
    'badgeText','heroTitle','logoDataUrl',
    'audioSizeMb','videoSizeMb','ecritSizeMb',
    'audioMaxMin','videoMaxMin',
    'storageType',
  ...
  // Durées max entières (minutes)
  ['audioMaxMin','videoMaxMin'].forEach(k => {
    if (updates[k] !== undefined) updates[k] = Math.max(1, Math.min(60, parseInt(updates[k]) || 10));
  });
...
function incrementStats(type) {
  try {
    ...
    const prev     = (cfg.stats && cfg.stats.month === nowMonth) ? cfg.stats : null;
    const stats    = prev
      ? { ...prev }
      : { month: nowMonth, audio: 0, video: 0, ecrit: 0, feedback: 0, test: 0 };
    stats[type] = (stats[type] || 0) + 1;
    setConfig({ stats });
  } catch(e) { console.warn('incrementStats:', e.message); }
}
...
app.get('/api/admin/stats', adminLimiter, requireAdmin, (req, res) => {
  ...
  const stats    = (cfg.stats && cfg.stats.month === nowMonth)
    ? cfg.stats
    : { month: nowMonth, audio: 0, video: 0, ecrit: 0, feedback: 0, test: 0 };
  const totalTemoignages = (stats.audio||0) + (stats.video||0) + (stats.ecrit||0) + (stats.feedback||0);
  const totalAvecTest    = totalTemoignages + (stats.test||0);
  res.json({ ok: true, stats, total: totalTemoignages, resendLimit: 3000, resendUsed: totalAvecTest });
});
...
app.post('/api/send/audio', sendLimiter, upload.single('audio'), async (req, res) => {
  try {
    ...
    const prenom   = String(req.body.prenom   || '').substring(0, 60);
    const nom      = String(req.body.nom      || '').substring(0, 60);
    const transcription = String(req.body.transcription || '').substring(0, 20000);
    ...
    if (req.file.size <= threshold || cfg.storageType === 'none') {
      ...
      emailMethod = 'cloud'; emailUrl = url;
      const { data, error } = await withTimeout(resend.emails.send({
        from:    fromHeader(cfg),
        to:      [cfg.destEmail],
    ...
    incrementStats('audio');

    // ── Supabase : stockage systématique (fichier + transcription + classement) ──
    let supa = null;
    if (isSupabaseEnabled()) {
      supa = await saveToSupabase({
        type: 'audio', prenom, nom, transcription,
        fileBuffer: req.file.buffer, filename, mime,
      });
      if (supa.ok) console.log(`🗂️  Supabase audio [${supa.id}] → ${supa.categorie || 'non classé'}`);
      else         console.warn(`🗂️  Supabase audio échoué : ${supa.error}`);
    }

    res.json({ ok: true, method: emailMethod, url: emailUrl, categorie: supa?.categorie || null });
...
app.post('/api/send/video', sendLimiter, upload.single('video'), async (req, res) => {
  try {
    ...
    const filename = safeFilename(req.body.filename, `${cfg.videoPrefix || 'video'}_${Date.now()}.webm`);
    const sender   = String(req.body.sender   || 'Anonyme').substring(0, 100);
    const prenom   = String(req.body.prenom   || '').substring(0, 60);
    const nom      = String(req.body.nom      || '').substring(0, 60);
    const transcription = String(req.body.transcription || '').substring(0, 20000);
    ...
    incrementStats('video');

    // ── Supabase : stockage systématique (fichier + transcription + classement) ──
    let supa = null;
    if (isSupabaseEnabled()) {
      supa = await saveToSupabase({
        type: 'video', prenom, nom, transcription,
        fileBuffer: req.file.buffer, filename, mime,
      });
      if (supa.ok) console.log(`🗂️  Supabase vidéo [${supa.id}] → ${supa.categorie || 'non classé'}`);
      else         console.warn(`🗂️  Supabase vidéo échoué : ${supa.error}`);
    }
...
app.post('/api/send/ecrit', sendLimiter, async (req, res) => {
  try {
    ...
    incrementStats('ecrit');

    // ── Supabase : la transcription écrite = le texte tapé (textContent) ──
    // Pas de fichier à uploader pour l'écrit.
    let supa = null;
    if (isSupabaseEnabled()) {
      supa = await saveToSupabase({
        type: 'ecrit', prenom, nom,
        transcription: textContent || '',
      });
      if (supa.ok) console.log(`🗂️  Supabase écrit [${supa.id}] → ${supa.categorie || 'non classé'}`);
      else         console.warn(`🗂️  Supabase écrit échoué : ${supa.error}`);
    }
