...
function defaults() {
  return {
    ...
    // ── Supabase ───────────────────────────────────────────────
    // L'URL et le nom du bucket peuvent vivre ici (non sensibles).
    // La service_role key reste UNIQUEMENT en variable d'environnement
    // (SUPABASE_KEY) — jamais dans config.json.
    supabaseUrl:    process.env.SUPABASE_URL     || '',
    supabaseBucket: process.env.SUPABASE_BUCKET  || 'temoignages',

    // ── Classement automatique ─────────────────────────────────
    // Mots-clés par catégorie, modifiables sans redéploiement.
    // Vide par défaut → categoriesConfig.js utilise sa liste interne.
    // Pour personnaliser : { "Guérison": ["guérir", ...], ... }
    categoriesKeywords: {},
  };
}
